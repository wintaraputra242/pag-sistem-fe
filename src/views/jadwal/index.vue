<template>
  <div class="dashboard-editor-container">
    <h1 style="text-align: center; font-family: Poppins; font-weight: 600;">JADWAL</h1>
    <div class="app-container">
      ...
      >
        ...
        <el-table-column label="Bobot" min-width="200">
          <template slot-scope="{row}">
            <span>{{ row.bobot }}</span>
          </template>
        </el-table-column>
        <el-table-column v-if="roleUser === '1'" label="Aksi" class-name="small-padding fixed-width" min-width="110" fixed="right">
          <template slot-scope="{row}">
            <el-button type="info" icon="el-icon-date" size="small" @click="getJadwal(row.id_user, row.nama)"> Jadwal </el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-dialog :title="'Jadwal ' + `(${nameUserJadwalSelected})`" :visible.sync="dialogJadwal" top="0px" style="padding: 20px 0;" :width="isWidthMobile ? '95%' : '60%'">
        <div>
          <p v-if="dataJadwal.length === 0" style="padding: 20px; margin: 0; text-align: center; border: 1px solid #cbd5e1; border-radius: 5px;">
            <i v-if="isLoadingJadwal" class="el-icon-loading" style="font-size: 20px;" />
            <span v-else>No Data</span>
          </p>
          <el-table
            v-else
            :key="tableKey"
            v-loading="isLoadingJadwal"
            :data="dataJadwal"
            highlight-current-row
            border
            stripe
          ...
          >
            ...
            <el-table-column label="Bobot" width="150px">
              <template slot-scope="{row}">
                <span>{{ row.bobot_text }}</span>
              </template>
            </el-table-column>
            <template v-for="(item, i) in labelDates">
              <el-table-column :key="i" :label="item" align="center" :class-name="'date-column'" width="25px">
                <template slot-scope="{row}">
                  <div style="width: 100% !important; height: 100% !important; margin: auto; position: absolute; top: 0" :style="{ background: row[item] }" />
                </template>
              </el-table-column>
            </template>
        ...
        <div slot="footer" class="dialog-footer">
          <el-button @click="dialogJadwal = false">
            Close
          </el-button>
        </div>
      </el-dialog>
...
export default {
  ...
  methods: {
    getList(params) {
      ...
      const { page, limit, bulanTahun } = this.filter
      const date = new Date()
      const nullFilterBulanTahun = (date.getMonth() + 1) + '-' + date.getFullYear()
      const [bulan, tahun] = !bulanTahun ? nullFilterBulanTahun.split('-') : bulanTahun.split('-')
      const newFilter = {
        page, limit, bulan: bulan || '', tahun: tahun || ''
      }
      this.$store.dispatch('userPusat/getDataUserJadwal', newFilter).then(response => {
        const userPusat = Object.values(this.$store.getters.dataUserForJadwal)
        const totalData = this.$store.getters.totalDataUserForJadwal
    ...
    getJadwal(id, nama) {
      this.nameUserJadwalSelected = nama
      this.dialogJadwal = true
      this.isLoadingJadwal = true
      this.dataJadwal = []
      this.totalDataJadwal = null
      this.filtersJadwal.id_user = id
      const { bulanTahun } = this.filter
      const date = new Date()
      const nullFilterBulanTahun = (date.getMonth() + 1) + '-' + date.getFullYear()
      const [bulan, tahun] = !bulanTahun ? nullFilterBulanTahun.split('-') : bulanTahun.split('-')
      this.filtersJadwal.bulan = bulan || new Date().getMonth() + 1
      this.filtersJadwal.tahun = tahun || new Date().getFullYear()
      this.$store.dispatch('userPusat/getDataJadwal', this.filtersJadwal).then(response => {
        const dataJadwal = this.$store.getters.dataJadwal
        // const totalData = this.$store.getters.totalTarget
        this.dataJadwal = dataJadwal
        this.totalDataJadwal = dataJadwal.length

        if (this.dataJadwal.length > 0) {
          this.labelDates = Object.keys(this.dataJadwal[0]).filter((val) => /^\d+$/.test(val))
          console.log(this.labelDates)
        }
